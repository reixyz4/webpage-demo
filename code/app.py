...
def predict():
    ...
    of2 = (os.path.join('..', 'soundscapes','of2.wav'))
# Export the modified audio to a new file
   ...
   # filename = 'nns.wav'
    ...
    data, rate = sf.read(output_file) # load audio (with shape (samples, channels))
    print(data.shape)
    meter = pyln.Meter(rate) # create BS.1770 meter
    loudness = meter.integrated_loudness(data) # measure loudness
    print("LOUD",loudness)
# loudness normalize audio to -37.8 dB LUFS
    loudness_normalized_audio = pyln.normalize.loudness(data, loudness, -37.8)
    sf.write(of2, loudness_normalized_audio, rate)
    
    data, rate = sf.read(of2) # load audio (with shape (samples, channels))
    print(data.shape)
    meter = pyln.Meter(rate) # create BS.1770 meter
    loudness = meter.integrated_loudness(data) # measure loudness
    print("NEW LOUD",loudness)
   ...
   # r1_dir = os.path.join('..','responses1')
    ...
    pt44= "The scores of the soundscape after masker is added"
    pt4 = "Plots the difference between the scores of the added-masker soundscape vs original soundscape"
    pt7="Positive difference means there is an increase in the score"
    return render_template('indexn.html', pt7=pt7, pt6 = pt6, pt5 = pt5, pt4 = pt4, pt3= pt3, pt2 = pt2, pt1 = pt1, prediction_text= prediction_text, table1=[tdf.to_html(classes='data', index = False)], titles1= tdf.columns.values, table2=[ndf.to_html(classes='data', index = False)], titles2= ndf.columns.values, table3=[odf.to_html(classes='data', index = False)], titles3= odf.columns.values,  table4=[rdf.to_html(classes='data', index = False)], titles4= rdf.columns.values, pt44 = pt44)
   # return render_template('indexn.html',
    #                      PageTitle = "Pandas",
...
# multiple line plot
  ...
  #  language = ['C', 'C++', 'Java', 'Python', 'PHP']
   # students = [23,11,32,22,11]
    array1 = np.array(res)
    array2 = np.array(ores)

    subtracted_array = np.subtract(array1, array2)
    subtracted = list(subtracted_array)

    plt.bar(x, subtracted)
   ...
   # plt.plot(x, y, color = 'red', label = "With Masker")
    ...
    plt.ylabel("Score Difference",size = 15)
